		...
		array(
			...
			'args'                => array(
				...
				'status' => array(
					...
					'enum'              => array( 'publish', 'draft', 'private', 'pending', 'trash' ),
				),
			),
		)
	);
...
function wptv_fetch_public_meta( array $post_ids ): array {
	...
	$meta = array();
	foreach ( $rows as $row ) {
		$meta[ (int) $row->post_id ][] = array( $row->meta_key, $row->meta_value );
	}
...
function wptv_bulk_status_handler( WP_REST_Request $request ): WP_REST_Response|WP_Error {
	...
	$ids    = array_merge( array( $id ), wptv_get_descendant_ids( $id ) );
	...
	return new WP_REST_Response( array( 'updated' => $ids ), 200 );
}
...
function wptv_duplicate_subtree_handler( WP_REST_Request $request ): WP_REST_Response|WP_Error {
	...
	if ( ! $original ) {
		return new WP_Error( 'wptv_not_found', 'Post not found.', array( 'status' => 404 ) );
	}
	...
	wptv_batch_insert_meta( $new_root->ID, $bulk_meta[ $original->ID ] ?? array() );

	$created = array( $new_root );
	// Map original ID → new ID so children can reference their new parent.
	$id_map = array( $original->ID => $new_root->ID );
	...
	foreach ( array_slice( $source_posts, 1 ) as $src ) {
		...
		wptv_batch_insert_meta( $new_post->ID, $bulk_meta[ $src->ID ] ?? array() );
